...
if [ ! -z "$CF_DOMAIN" ]; then
    cat >> "$PROJECT_ROOT/DEPLOYMENT_INFO.md" <<EOF
- **웹사이트 (CloudFront)**: https://${CF_DOMAIN}
EOF
else
    cat >> "$PROJECT_ROOT/DEPLOYMENT_INFO.md" <<EOF
- **웹사이트 (S3)**: http://${S3_BUCKET}.s3-website-${REGION}.amazonaws.com
EOF
fi
...
echo ""
if [ ! -z "$CF_DOMAIN" ]; then
    log_success "🌐 웹사이트 URL (CloudFront): https://$CF_DOMAIN"
else
    log_success "🌐 웹사이트 URL (S3): http://${S3_BUCKET}.s3-website-${REGION}.amazonaws.com"
fi
