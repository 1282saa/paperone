...
log_info "WebSocket API Gateway 설정 시작..."
...
if [ -z "$WS_API_ID" ]; then
    ...
    log_info "기존 WebSocket API 사용: $WS_API_ID"
    # 기존 라우트 삭제
    log_info "기존 라우트 정리 중..."
    EXISTING_ROUTES=$(aws apigatewayv2 get-routes --api-id "$WS_API_ID" --region "$REGION" --query 'Items[].RouteId' --output text)
    for route_id in $EXISTING_ROUTES; do
        aws apigatewayv2 delete-route --api-id "$WS_API_ID" --route-id "$route_id" --region "$REGION" 2>/dev/null || true
    done
fi
...
create_route() {
    ...
    if [ -z "$integration_id" ] || [ "$integration_id" = "null" ]; then
        log_error "Integration ID가 없습니다: $route_key"
        return 1
    fi
...
if [ -z "$STAGE_EXISTS" ]; then
    ...
    aws apigatewayv2 update-stage \
        ...
        --deployment-id "$DEPLOYMENT_ID" \
        --region "$REGION" >/dev/null
    log_success "WebSocket API 스테이지 업데이트 완료!"
fi
