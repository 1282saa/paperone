...
# nexus-template-v2용 f1-two 스택

set -e  # 에러 발생시 즉시 중단

CARD_COUNT="two"
STACK_NAME="f1-${CARD_COUNT}"
...
# Lambda 함수 이름들
LAMBDA_CONVERSATIONS="f1-conversation-api-${CARD_COUNT}"
LAMBDA_PROMPTS="f1-prompt-crud-${CARD_COUNT}"
LAMBDA_USAGE="f1-usage-handler-${CARD_COUNT}"
...
REST_API_ID=$(aws apigateway create-rest-api \
  --name "f1-rest-api-${CARD_COUNT}" \
  --description "F1 Service REST API - Complete" \
...
WS_API_ID=$(aws apigatewayv2 create-api \
  --name "f1-websocket-api-${CARD_COUNT}" \
...
# Connect route
WS_CONNECT_LAMBDA="f1-websocket-connect-${CARD_COUNT}"
...
# Disconnect route
WS_DISCONNECT_LAMBDA="f1-websocket-disconnect-${CARD_COUNT}"
...
# Default route
WS_MESSAGE_LAMBDA="f1-websocket-message-${CARD_COUNT}"
...
aws apigateway get-export \
  ...
  --stage-name prod \
  --export-type swagger \
  f1-api-spec-final.json 2>/dev/null || echo "  (스펙 내보내기 생략)"
